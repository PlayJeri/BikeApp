...
def test_get_station_404(client, test_stations):
    station_id = 123
    res = client.get(f"/station/{station_id}")

    assert res.status_code == 404

def test_search_stations(client, test_stations):
...
def test_search_stations_empty(client, test_stations):
    res = client.get("/stations/search?station_name=empty")
    data = res.json()

    assert(data == [])

def test_search_stations_limit(client, test_stations):
    res = client.get("/stations/search?station_name=station")
    data = res.json()

    assert(len(data) == 3)

def test_search_station_multiple(client, test_stations):
